...
%% -------------------- YOUR SCALING (edit if needed) --------------------
% ESP DAC u_esp in [0,3.3] -> Station input u_st in [1,5]
ku = 4/3.3;    bu = 1.0;                 % u_st = bu + ku*u_esp
...
STEP_THRESH_V = 0.01;   % step detect threshold on setpoint_v (raw)
...
% Make it safely larger than the largest model “memory”.
...
%% -------------------- CONVERT TO STATION VOLTS (1..5V) ----------------
% Setpoint: ESP-side (<=3.3) or already station-side (<=5)
if max(sp_raw) <= 3.35
    u_st = bu + ku*sp_raw;           % sp_raw is u_esp
    fprintf("Input: setpoint_v treated as ESP DAC (0..3.3V) -> station input (1..5V)\n");
else
    u_st = sp_raw;                   % already station volts
    fprintf("Input: setpoint_v treated as station input (1..5V)\n");
end

% Sensor: ADC-side (<=3.3) or already station-side (<=5)
if max(y_raw) <= 3.35
    y_st = (y_raw - by)/ky;          % ADC -> station output
    fprintf("Output: sensor_v treated as ADC (0.67..3.3V) -> station output (1..5V)\n");
else
    y_st = y_raw;
    fprintf("Output: sensor_v treated as station output (1..5V)\n");
end

%% -------------------- FIND STEP INDEX --------------------
dsp = [0; diff(sp_raw)];
stepIdx = find(abs(dsp) > STEP_THRESH_V, 1, "first");

if isempty(stepIdx)
    du_try = [0; diff(u_st)];
    stepIdx = find(abs(du_try) > STEP_THRESH_V, 1, "first");
end
if isempty(stepIdx)
...
% Use whatever pre-step exists to compute baseline; if none, use early samples.
preAvail = stepIdx - 1;
...
% current pre-step count = stepIdx-1
...
if needPad > 0
    ...
    % Also pad raw absolute signals for plotting convenience
    ...
    % rebuild time vector to match Ts (safer for iddata)
    t = (0:Ts:Ts*(length(du)-1))';
end
...
% IMPORTANT: Avoid polydata() (your error). Read coefficients directly.
A  = bestSys.A;
B  = bestSys.B;
C  = bestSys.C;
nk = bestSys.nk;
...
% Residual check (optional but useful)
