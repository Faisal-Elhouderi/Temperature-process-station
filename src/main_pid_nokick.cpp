...
//
//  What you asked for:
//    - P and D use ONLY the measured output y (not the error)
//    - I uses the error e = r - y
//
//  This is a common 2DOF structure (I-PD / setpoint weighting):
//    u = Kp*(b*r - y) + Ki*\int(r-y)dt - Kd*d(y_f)/dt
//  With b=0  =>  P = -Kp*y  (no proportional kick from setpoint steps)
//  And D on measurement => no derivative kick from setpoint steps
//
//  Notes:
//   - Controller is implemented in discrete time (difference equations),
//     but you can keep your s-domain parameters (Kp, Ti, Td, N) and set Ts.
//   - Includes: band-limited derivative (N), moving-average ADC (10 samples at 0.5 s),
//     anti-windup + output saturation, and bumpless enable.
// =========================================================
...
//  b = 0.0  => P uses ONLY output (-Kp*y)  (your requested "kick fix")
//  b = 1.0  => classic P on error (Kp*(r-y))
// For temperature control, b in [0.2..0.8] is often a nice compromise.
...
float Td = 0.0f;       // seconds  (Td=0 => PI / I-P)
...
// PID states
float iState = 0.0f;         // integrator contribution (in volts)
...
//  - Trapezoidal (Tustin) integration for I on error
//  - First-order low-pass filtered derivative on measurement for D
//  - P on (b*r - y) with b=0 by default (your requested no-kick structure)
//  - Conditional integration anti-windup (freeze integrator when pushing deeper into saturation)
//
float pidStepNoKick(float rV, float yV, float Ts,
  ...
  // --- P (setpoint weighted, b=0 by default) ---
  // If SETPOINT_WEIGHT_B == 0: P = -Kp*y (no proportional kick)
  ...
  // --- I (candidate update using trapezoidal integration on error) ---
  ...
  if (satFlag) {
    // If saturated AND error would drive output further into saturation, freeze integrator.
...
void initPidBumpless(float rV, float yV, float currentU) {
  // Reset derivative state so first derivative is ~0
  dFilt = 0.0f;
  prevY = yV;

  // Initialize prevError for trapezoidal I
  prevError = (rV - yV);

  // Compute P and D with these initial states
  float p0 = Kp * (SETPOINT_WEIGHT_B * rV - yV);
  float d0 = 0.0f; // because dFilt=0 and prevY=yV

  // Choose iState so u = p + i + d == currentU
  float i0 = currentU - p0 - d0;

  // Keep iState within range that can produce a valid saturated output
...
void setup() {
  ...
  // Initialize states (PID still disabled)
  float y0 = readSensorVoltageAveraged();
  initPidBumpless(refSensorV, y0, uOutV);
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    switch (cmd) {
      ...
      case 'E': {
        ...
        if (pidEnabled) {
          // bumpless init so u doesn't jump when enabling
          initPidBumpless(refSensorV, yNow, uOutV);
          Serial.println("PID ENABLED (bumpless, kick-reduced I-PD)");
        } else {
  ...
  if (now - lastSampleTime >= SAMPLE_INTERVAL_MS) {
    ...
    if (pidEnabled && (sampleCount % PID_UPDATE_EVERY_N_SAMPLES == 0)) {
      ...
      float uCmdV = pidStepNoKick(refSensorV, yAvgV, Ts, pTerm, iTerm, dTerm, sat);
      setDacVoltage(uCmdV);
